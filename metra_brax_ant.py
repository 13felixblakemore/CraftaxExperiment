...
def get_root_xy(env_state) -> jax.Array:
    """Global Ant torso x/y coordinates, with any leading batch dimensions."""
    return env_state.pipeline_state.x.pos[..., 0, :2]


def get_metra_obs(env_state) -> jax.Array:
    """Brax Ant observation with omitted global torso x/y appended."""
    return jnp.concatenate([env_state.obs, get_root_xy(env_state)], axis=-1)


def _phi_input(obs: jax.Array, config) -> jax.Array:
    """Optionally restrict phi to global x/y for a spatial sanity check."""
    if config.get("PHI_XY_ONLY", True):
        return obs[..., -2:]
    return obs


def make_train(config):
    """Builds the same METRA + SAC update on the Brax Ant environment."""

    requested_name = str(config.get("ENV_NAME", "ant")).lower()
    if requested_name not in {"ant", "brax_ant", "braxant"}:
        raise ValueError(
            f"This module only supports Brax Ant, received ENV_NAME={requested_name!r}."
        )

    base_env = brax_envs.get_environment(
        env_name="ant",
        backend=config.get("BRAX_BACKEND", "mjx"),
        exclude_current_positions_from_observation=True,
        terminate_when_unhealthy=config.get("TERMINATE_WHEN_UNHEALTHY", False),
        reset_noise_scale=config.get("RESET_NOISE_SCALE", 0.1),
    )

    episode_length = int(config.get("EPISODE_LENGTH", 200))
    action_repeat = int(config.get("ACTION_REPEAT", 1))

    # Vmap + time limit + cached auto-reset, matching the Playground training loop.
    env = brax_envs.training.wrap(
        base_env,
    ...
    def train(rng):
        ...
        phi_params = phi.init(phi_key, _phi_input(dummy_obs, config))
        ...
        # Ant's built-in reward is logged only. METRA/SAC trains on the
        ...
        def train_loop(carry, iteration):
            ...
            def update(carry, _):
                ...
                def metra_components(current_phi_params):
                    phi_obs = phi.apply(
                        current_phi_params, _phi_input(obs_b, config)
                    )
                    phi_next = phi.apply(
                        current_phi_params, _phi_input(next_obs_b, config)
                    )
...
def _make_raw_eval_env(config):
    return brax_envs.get_environment(
        env_name="ant",
        backend=config.get("BRAX_BACKEND", "mjx"),
        exclude_current_positions_from_observation=True,
        terminate_when_unhealthy=config.get("TERMINATE_WHEN_UNHEALTHY", False),
...
def _root_xyz(state) -> np.ndarray:
    return np.asarray(
        jax.device_get(state.pipeline_state.x.pos[0, :3]), dtype=np.float64
    )
...
def evaluate_skill_diagnostics(
    ...
    phi_state: TrainState,
) -> dict:
    """Deterministic Ant diagnostics using matched reset seeds for every skill."""
    env = _make_raw_eval_env(config)
    actor = Actor(
        config["LAYER_SIZE"],
        env.action_size,
    ...
    def encode(params, obs):
        return encoder.apply(params, _phi_input(obs, config))
    ...
    for skill_i in range(num_skills):
        z = eval_zs[skill_i]
        z_host = np.asarray(jax.device_get(z), dtype=np.float64)
        reward_vector = np.asarray(jax.device_get(reward_vectors[skill_i]), dtype=np.float64)
        rollouts = []
        all_phi_diffs = []

        for rollout_i in range(num_rollouts):
            # Same initial-state set for every skill.
            state = reset(jax.random.fold_in(base_key, rollout_i))
            initial_xyz = _root_xyz(state)
            previous_xy = initial_xyz[:2]
            xy = [previous_xy.copy()]
            heights = [initial_xyz[2]]
            velocities = []
            actions = []
            phi_rewards = []
            env_return = 0.0
            ...
            for t in range(horizon):
                ...
                for _ in range(action_repeat):
                    ...
                    xyz = _root_xyz(state)
                    current_xy = xyz[:2]
                    velocities.append((current_xy - previous_xy) / float(env.dt))
                    previous_xy = current_xy
                    xy.append(current_xy.copy())
                ...
                phi_diff = np.asarray(
                    jax.device_get(next_phi - previous_phi), dtype=np.float64
                )
                all_phi_diffs.append(phi_diff)
                phi_rewards.append(float(phi_diff @ reward_vector))
            ...
            if not config.get("DISCRETE", False) and config["Z_DIM"] == 2:
                direction_alignment = float(
                    delta_xy @ z_host / (np.linalg.norm(delta_xy) * np.linalg.norm(z_host) + 1e-8)
                )
            ...
            rollouts.append({
                ...
                "mean_vx": velocities[:, 0].mean() if velocities.size else np.nan,
                "mean_vy": velocities[:, 1].mean() if velocities.size else np.nan,
                "mean_speed": np.linalg.norm(velocities, axis=-1).mean() if velocities.size else np.nan,
                "xy_path_length": np.linalg.norm(np.diff(xy, axis=0), axis=-1).sum(),
                "x_range": np.ptp(xy[:, 0]),
                "y_range": np.ptp(xy[:, 1]),
        ...
        row = {
            ...
            "delta_y_std": std("delta_y"),
            "distance": mean("distance"),
            "direction_alignment": mean("direction_alignment"),
            "mean_vx": mean("mean_vx"),
            "mean_vy": mean("mean_vy"),
            "mean_speed": mean("mean_speed"),
            "xy_path_length": mean("xy_path_length"),
        ...
        rows.append(row)
        skill_mean_phi_diffs.append(np.mean(all_phi_diffs, axis=0))

        print(
            f"Skill {skill_i:02d} | dxy=({row['delta_x']:+.2f},{row['delta_y']:+.2f}) "
            f"dist={row['distance']:.2f} align={row['direction_alignment']:+.2f} "
            f"speed={row['mean_speed']:.2f} path={row['xy_path_length']:.2f} "
            f"height={row['mean_height']:.2f}±{row['std_height']:.2f} "
            f"energy={row['action_energy']:.3f} phi_r={row['mean_phi_reward']:+.3f}"
        )

    # Alignment of each generated skill trajectory with every reward vector.
    ...
    endpoints = np.asarray([[r["delta_x"], r["delta_y"]] for r in rows])
    summary = {
        "mean_endpoint_distance": float(np.mean([r["distance"] for r in rows])),
        "mean_pairwise_endpoint_distance": _mean_pairwise_distance(endpoints),
        "mean_direction_alignment": float(np.nanmean([r["direction_alignment"] for r in rows])),
        "mean_xy_path_length": float(np.mean([r["xy_path_length"] for r in rows])),
        ...
        "mean_termination_rate": float(np.mean([r["termination_rate"] for r in rows])),
    }

    output_dir = Path(config.get("EVAL_VIDEO_DIR", "eval_videos_brax_ant"))
    output_dir.mkdir(parents=True, exist_ok=True)
    _write_csv(output_dir / "eval_skills.csv", rows)
    ...
    print(
        ...
        f"\n  mean endpoint distance: {summary['mean_endpoint_distance']:.3f}"
        f"\n  mean pairwise endpoint distance: {summary['mean_pairwise_endpoint_distance']:.3f}"
        f"\n  mean direction alignment: {summary['mean_direction_alignment']:.3f}"
...
def record_eval_videos(
    ...
    """Runs diagnostics and writes one deterministic MP4 for each Ant skill."""
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    base_key = jax.random.PRNGKey(int(config.get("EVAL_SEED", config.get("SEED", 0))))
    reset_index = int(config.get("EVAL_VIDEO_RESET_INDEX", 0))
    paths = []

    for skill_i, z in enumerate(eval_zs):
        state = reset(jax.random.fold_in(base_key, reset_index))
        pipeline_rollout = [state.pipeline_state]
        initial_xy = _root_xyz(state)[:2]
        env_return = 0.0
        ...
        final_xy = _root_xyz(state)[:2]
        frames = env.render(
            pipeline_rollout,
            height=int(config.get("EVAL_HEIGHT", 480)),
            width=int(config.get("EVAL_WIDTH", 640)),
            camera=config.get("EVAL_CAMERA", None),
        )
        path = output_dir / f"brax_ant_skill_{skill_i:02d}.mp4"
        media.write_video(
            str(path), frames, fps=1.0 / (float(env.dt) * action_repeat)
        )
        print(
            f"Recorded skill {skill_i:02d}: return={env_return:.2f}, "
            f"dxy=({final_xy[0] - initial_xy[0]:+.2f},"
            f"{final_xy[1] - initial_xy[1]:+.2f}), path={path}"
        )
        paths.append(path)
