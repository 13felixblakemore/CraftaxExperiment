    ...
    def train(rng):
        ...
        def update_step(run_state, _):
            def rollout_step(carry, transition):
                ...
                all_actions = policy.sample(seed=_rng)
                actions = all_actions[jnp.arange(config["NUM_ENVS"]), option]
                log_probs = policy.log_prob(all_actions)
            ...
            def compute_gae_all(rollout, last_q, last_b, option):
                def gae_step(carry, transition):
                    ...
                    next_non_terminal = jnp.expand_dims(next_non_terminal, 1)

                    bootstrap = ((1 - next_b[jnp.arange(config["NUM_ENVS"])]) * next_value[jnp.arange(config["NUM_ENVS"])] + next_b[jnp.arange(config["NUM_ENVS"])] * jnp.max(next_value, axis=-1, keepdims=True))

                    delta = reward + config["GAMMA"] * bootstrap * next_non_terminal - value[jnp.arange(config["NUM_ENVS"])]
                ...
                initial_carry = (
                    jnp.zeros((config["NUM_ENVS"], config["NUM_OPTIONS"]), dtype=jnp.float32),
                    last_q,
                    last_b
                )
            ...
            advantages, returns = compute_gae_all(rollout, q_w_next, b_next, option)
            ...
            def update_epoch(update_state, _):
                def update_minibatch(train_state, batch_info):
                    ...
                    def loss_fn(params, rollout, gae, returns):
                        ...
                        greedy_one_hot = jax.nn.one_hot(
                            greedy_options,
                            num_classes=config["NUM_OPTIONS"],
                        ) # 256, 4
                        ...
                        option_probs = (
                                config["OPTION_POLICY_EPS"] / config["NUM_OPTIONS"]
                                + (1.0 - config["OPTION_POLICY_EPS"]) * greedy_one_hot
                        )

                        beta = b[jnp.arange(config["MINIBATCH_SIZE"]), options, None]
